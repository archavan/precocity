...
# model
pgls_consensus <- gls(
  log(gestation_len_d) ~ log(adult_body_mass_g) + precocity,
  data = prec_data,
...
pgls_consensus_bm <- gls(
  log(gestation_len_d) ~ log(adult_body_mass_g),
  data = prec_data,
  correlation = cor_lambda_consensus,
  na.action = na.exclude
)
...
segment_coords_pgls_altrial <- calc_segment_coords(
  .xmin = min(log(prec_data$adult_body_mass_g[which(
    prec_data$precocity == "altricial"
  )])),
  .xmax = max(log(prec_data$adult_body_mass_g[which(
    prec_data$precocity == "altricial"
  )])),
...
segment_coords_pgls_intermediate <- calc_segment_coords(
  .xmin = min(log(prec_data$adult_body_mass_g[which(
    prec_data$precocity == "intermediate"
  )])),
  .xmax = max(log(prec_data$adult_body_mass_g[which(
    prec_data$precocity == "intermediate"
  )])),
...
gl_vs_bm <- ggplot(
  ...
  geom_segment(
    x = segment_coords_pgls_altrial$x,
  ...
  geom_segment(
    x = segment_coords_pgls_intermediate$x,
  ...
  geom_segment(
    x = segment_coords_pgls_precocial$x,
...
# save plots ==================================================================
ggsave(
  here(resdir, "supp-fig_pgls_v2.png"),
  plot = bm_only_patch,
  width = 6.5,
  height = 3,
  dpi = 600
