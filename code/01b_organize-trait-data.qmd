...
# traits
kd <- read_excel(
  here("data/traits/MP-KD/MammaliaData_MP_KD_20161207.xlsx"),
  na = c("", "NA")
)
pt <- read_tsv(
  here("data/traits/PanTHERIA/PanTHERIA_1-0_WR05_Aug2008.txt"),
  na = c(-999, "-999", "", "NA"),
  guess_max = 10000
)
mv <- read_csv(
  here("data/traits/Myhrvold/Amniote_Database_Aug_2015.csv"),
  na = "-999",
  guess_max = 10000
)
...
# curated synonyms mapping older binomials onto Upham 2019 tip labels
synonyms <- read_csv(here("data/taxa/synonyms_upham2019.csv"))
```
...
```{r}
spp <- bind_rows(
  myhrvold = select(mv, family, binomial),
...
spp_wide <- spp_wide %>%
  mutate(
    myhrvold = str_replace(myhrvold, "Indridae", "Indriidae"),
...
## Add the Upham 2019 tip label for each species

The binomials above follow MSW3, which the trait datasets use. The Upham 2019 tree uses newer nomenclature for some species. We add the tip label as a new column

```{r}
spp_upham <- spp_resolved %>%
  left_join(synonyms, by = c("binomial" = "binomial_synonym")) %>%
  mutate(binomial_upham19 = coalesce(binomial_upham19, binomial)) %>%
  mutate(
    binomial_upham19 = ifelse(
      binomial_upham19 %in% up19$binomial,
      binomial_upham19,
      NA_character_
    )
  ) %>%
...
# the map must not collapse two species onto one tip
stopifnot(!any(duplicated(na.omit(spp_upham$binomial_upham19))))

# every non-missing tip label must actually be in the tree
stopifnot(all(na.omit(spp_upham$binomial_upham19) %in% up19$binomial))

count(spp_upham, in_tree = !is.na(binomial_upham19))
```
...
write_csv(spp_upham, here("data/taxa/species.csv"))
```
