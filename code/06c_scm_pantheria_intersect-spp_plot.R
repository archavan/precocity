...
analysis_name <- "pantheria-intersect-spp"
...
prec_data <- prec_data %>% 
  mutate(precocity_pt = fct(precocity_pt, 
                            c("altricial", "intermediate", "precocial")))
prec_tipdata <- set_names(prec_data$precocity_pt, prec_data$binomial)
...
asr <- tibble(tr_id = 1:96, tr_name = dir(here(resdir, "sample")))
