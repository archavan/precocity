...
up19 <- read.nexus(
  "data/trees/upham2019/consensus/DNA-only/MamPhy_fullPosterior_BDvr_DNAonly_4098sp_topoFree_NDexp_MCC_v2_target.tre"
)
...
tr_consensus <- up19
tr_consensus$tip.label <- up19_tips$binomial[match(
  tr_consensus$tip.label,
  up19_tips$tiplab
)]

stopifnot(all(map2_lgl(
  tr_consensus$tip.label,
  up19$tip.label,
  ~ str_detect(.y, .x)
)))
