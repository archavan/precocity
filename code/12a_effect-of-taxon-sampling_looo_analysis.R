...
df <- df %>%
  ...
  mutate(
    simmap_consensus = map(fit_aov, ~ run_simmap(.x, 1000), .progress = TRUE)
  ) %>%
  mutate(
    simmap_consensus_summary = map(simmap_consensus, summary, .progress = TRUE)
  ) %>%
