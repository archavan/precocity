...
case78_plus_pt <- full_join(
  ...
  select(-precocity_pt, -precocity_case78) %>%
  distinct()
...
write_csv(
  arrange_taxa(shared),
  here("data/03_coded/pt_case78_intersect-spp.csv")
)
write_csv(
  arrange_taxa(pt_plus_underrep),
  here("data/03_coded/pantheria-plus-underrep-taxa.csv")
)
write_csv(
  arrange_taxa(case78_plus_underrep),
  here("data/03_coded/case78-plus-underrep-taxa.csv")
)
write_csv(
  arrange_taxa(case78_plus_pt),
  here("data/03_coded/case78-plus-pantheria.csv")
)
