...
plot_pp_dist_for_clade <- function(.clade) {
  pp |>
    ...
    scale_y_discrete(name = "Threshold combination") +
    scale_fill_manual(
      values = clrs,
      breaks = names(clrs),
...
# plot pp density for eutheria in a grid ======================================

pp_dist_grid <- pp |>
  filter(clade == "Eutheria") |>
  left_join(
    select(taxonsets, taxonset, eye_upper, litter_upper),
    by = "taxonset"
  ) |>
  pivot_longer(
    cols = all_of(names(clrs)),
  ...
  ggplot(aes(pprob, fill = precocity)) +
  geom_histogram(
    position = "dodge",
    binwidth = 0.075,
    boundary = 0.5,
  ...
  facet_grid(
    rows = vars(eye_upper),
    cols = vars(litter_upper),
    scales = "free",
    labeller = labeller(
      litter_upper = function(x) {
        paste0("Litter size: ", x)
      },
      eye_upper = function(x) {
        paste0("Eyes open (d): ", x)
      }
    )
  ) +
  scale_fill_manual(
  ...
  scale_x_continuous("PP at Eutheria node") +
  scale_y_continuous("Count") +
  theme_bw() +
  theme(strip.background = element_blank(), panel.grid = element_blank())

pdf(
  here(resdir, "pp-distribution-grid_eutheria.pdf"),
  width = 7,
  height = 5.5
)
print(pp_dist_grid)
