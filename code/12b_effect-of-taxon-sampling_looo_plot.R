...
orders_txinfo <- prec_data %>%
  select(rank01, rank02, rank03, rank04, rank05, rank06, rank07) %>%
  distinct()

df <- right_join(orders_txinfo, df, by = c("rank07" = "leave_out"))
...
get_node <- function(.phy, .taxon) {
  ...
  if (!all(spp_list %in% .phy$tip.label)) {
    warning(
      "Some species not found in the tree. Make sure you know what you are doing!"
    )
...
ace_pp_plot <- ggplot(ace_pp, aes(pp, rank07, fill = precocity)) +
  geom_col(color = "black", linewidth = 0.25) +
  scale_fill_manual(values = clrs) +
  theme_half_open(font_family = "Source Sans Pro") +
  labs(x = "PP at node Eutheria", y = "Order left out")

ggsave(
  here(resdir, "eutheria_ace_pp.pdf"),
  ace_pp_plot,
  width = 7,
  height = 7,
  device = cairo_pdf
