...
png(
  here(figdir, "supp-fig_scm_case78_a_consensus-asr.png"),
  width = 10,
  height = 5.5,
  units = "in",
  res = 600,
...
ggsave(
  here(figdir, "supp-fig_scm_case78_composed_v1.png"),
  composed,
  width = 6.5,
  height = 5,
  units = "in",
...
p_delta_aic_sample <- mw_sample %>%
  ...
  geom_segment(
    aes(x = tr_id, xend = tr_id, y = 0, yend = delta_aic),
    linewidth = 0.25
  ) +
  geom_point(size = 0.5) +
...
model_fit_effect <- mw_sample %>%
  ...
  mutate(
    pp_eutheria = map2(
      ace,
      tr_pruned,
      ~ get_pp_at_node(.x, get_node(.y, "Eutheria"))
    )
  ) %>%
...
ggsave(
  here(figdir, "supp-fig_scm_case78_mw-aic.png"),
  mw_aic_patch,
  width = 6.5,
  height = 2.4,
  units = "in",
...
ggsave(
  here(figdir, "supp-fig_scm_case78_model-choice.png"),
  model_fit_effect,
  width = 3,
  height = 2.25,
  units = "in",
...
mw_sample %>%
  mutate(delta_aic = aic - min(aic), .by = tr_id) %>%
  group_by(model) %>%
  summarise(mean_weight = mean(weight), mean_delta_aic = mean(delta_aic)) %>%
  ungroup()
