...
prec_data |>
  count(rank07, rank08, family) |>
  write_csv(here(resdir, "families.csv"))

## generate receptacle for results ============================================
families <- unique(prec_data$family)

df <- tibble(leave_out = families)

df <- df |>
  mutate(prec = map(leave_out, ~ filter(prec_data, family != .x))) |>
