...
alt_comp <- ggplot(prec, aes(log10(bm_ratio), fill = precocity)) +
  ...
  theme_bw(base_family = "Source Sans 3", base_size = 6) +
  theme(
...
ggsave(
  here(resdir, "fig_altriciality-comparison.pdf"),
  alt_comp,
  width = 3,
  height = 2.5,
  device = cairo_pdf
...
ggsave(
  here(resdir, "fig_altriciality-comparison.png"),
  alt_comp,
  width = 3,
  height = 2.5,
  dpi = 600,
...
ggsave(
  here(resdir, "fig_altriciality-comparison_by-order.pdf"),
  bm_ratio_by_order,
  width = 3.5,
  height = 3.7,
  device = cairo_pdf
...
bm_ratio_pct <- ggplot(prec, aes(bm_ratio * 100, rank07, fill = precocity)) +
  ...
  labs(x = "Relative Neonatal Body Mass (%)", y = "Order") +
  theme_bw(base_family = "Source Sans 3", base_line_size = 0.25) +
  theme(
    strip.clip = "off",
...
ggsave(
  here(resdir, "fig_altriciality-comparison_by-order_pct.pdf"),
  bm_ratio_pct,
  width = 3.1,
  height = 3.1,
  device = cairo_pdf
...
ggsave(
  here(resdir, "fig_altriciality-comparison_by-order_pct.png"),
  bm_ratio_pct,
  width = 3.1,
  height = 3.1,
  dpi = 600,
