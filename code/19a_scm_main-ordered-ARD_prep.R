...
## parameters =================================================================
analysis <- "main-ordered-ARD"
model_to_test <- "custom"
nsim <- 1000
...
ordered_ard <- matrix(
  ...
  ncol = 3,
  byrow = TRUE,
  dimnames = list(levels(tipdata), levels(tipdata))
)
...
custommodel_path <- here(resdir, "custommodel.rds")
...
write_rds(ordered_ard, custommodel_path, compress = "gz")
...
--model {model_to_test} \
--custommodel {fs::path_rel(custommodel_path, here())} \
--nsim {nsim}
  ]"
)
