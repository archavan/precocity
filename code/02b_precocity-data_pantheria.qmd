...
pt <- pt %>%
  mutate(
    age_at_eye_opening_d = case_when(
...
pt_non_na <- pt_non_na %>%
  ...
  mutate(
    litter_size_discrete = factor(
      litter_size_discrete,
      levels = c("low", "medium", "high")
    ),
    age_at_eye_opening_discrete = factor(
...
```{r}
table(
  pt_non_na$litter_size_discrete,
  pt_non_na$age_at_eye_opening_discrete,
  dnn = c("litter_size_discrete", "age_at_eye_opening_discrete")
)
```
...
```{r}
fs::dir_create(here("data/03_coded/pantheria/manual"))
pt_coded_algorithmic %>%
  filter(is.na(precocity)) %>%
  write_csv(here("data/03_coded/pantheria/manual/to-code.csv"))
```
...
manually_coded <- read_csv(here("data/03_coded/pantheria/manual/coded.csv"))
...
pt_coded %>%
  mutate(
    text_label = ifelse(coding_method == "manual", binomial, NA_character_)
  ) %>%
  ggplot(aes(adult_body_mass_g, gestation_len_d, label = text_label)) +
  geom_point(aes(shape = coding_method, color = precocity), size = 2) +
...
tax_coverage <- tax_coverage %>%
  mutate(
    cvg_base = map_dbl(order, ~ calc_fraction(spp, .x)),
    cvg_pt_coded = map_dbl(order, ~ calc_fraction(pt_coded, .x))
  ) %>%
  mutate(enrichment = cvg_pt_coded / cvg_base) %>%
  arrange(enrichment)

print(tax_coverage, n = Inf)
```

There are 17 orders that are underrepresented, but 10 orders will be severely underrepresented in the precocity data (less than 0.1 enrichment).

```{r}
# data for all species in orders that will be underrepresented.
underrep <- spp %>%
  filter(
    rank07 %in% tax_coverage$order[which(tax_coverage$enrichment < 0.1)]
  ) %>%
...
```{r}
write_csv(pt_coded, here("data/03_coded/pantheria/precocity_pantheria_v1.csv"))

write_csv(tax_coverage, here("data/03_coded/pantheria/taxonomic-coverage.csv"))
write_csv(
  underrep,
  here("data/03_coded/pantheria/spp-from-underrepresented-orders.csv")
)
```
