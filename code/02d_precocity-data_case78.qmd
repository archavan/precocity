...
case78 <- case78 %>%
  mutate(
    binomial = ifelse(!is.na(synonym_upham19), synonym_upham19, binomial_case78)
  )
...
get_family <- function(.synonyms, .data = spp) {
  ...
  repeat {
    if (i > length(.synonyms) | length(fam) != 0) {
      break
    }
...
case78 <- case78 %>%
  mutate(
    family = map_chr(synonyms, possibly(~ get_family(.x, spp), NA_character_))
  )
...
write_csv(
  underrep,
  here("data/03_coded/case78/spp-from-underrepresented-orders.csv")
)
```
