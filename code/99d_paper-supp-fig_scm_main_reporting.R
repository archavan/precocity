...
ggsave(
  here(figdir, "supp-fig_scm_main_reporting.png"),
  composed,
  width = 6.2,
  height = 7.2,
  units = "in",
...
prettify_mw_anova(asr$fit[[which(asr$treetype == "consensus")]]) |>
  rename(
    `Model` = model,
    `Log Lik.` = log_l,
    `D.F.` = d_f,
  ...
  write_rds(here(figdir, "supp-tbl_mw_main_consensus.rds"))

# fitted rates tbale for consensus tree =======================================

rates |>
  filter(treename == "consensus") |>
  select(transition, rate, model) |>
  rename(Transition = transition) |>
  pivot_wider(names_from = "model", values_from = "rate") |>
  mutate(Transition = str_to_title(Transition)) |>
  write_rds(here(figdir, "supp-tbl_rates_main_consensus.rds"))
